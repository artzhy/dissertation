...
namespace WebService {
    ...
    public interface IApplicationSvc {
        ...
        BusinessLayer.WorkApplication AddApplication(AuthToken at, string applicationName, string applicationDescription, string applicationCreator, string applicationPackageUrl, string applicationWorkIntent, string applicaitonNamespace, int applicationVersion = 1);

        [OperationContract]
        [WebGet(UriTemplate = "RegisterDeviceToApp", ResponseFormat = WebMessageFormat.Json, RequestFormat = WebMessageFormat.Json)]
        BusinessLayer.DeviceAppInstallation RegisterDeviceToApp(AuthToken at, int applicationId, int deviceId);

        [OperationContract]
        [WebGet(UriTemplate = "DeregisterDeviceToApp", ResponseFormat = WebMessageFormat.Json, RequestFormat = WebMessageFormat.Json)]
        void DeregisterDeviceToApp(AuthToken at, int applicationId, int deviceId);
